...
interface TenantData {
  id: string;
  subdomain: string;
  emoji: string; // tidak dipakai lagi tapi disimpan agar tidak error di tipe
  businessName: string;
  businessCategory: string; // tidak dipakai lagi
  ownerName: string;
  email: string;
  phone: string;
  address?: string;
...
export default function BeautyTemplate({ 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-purple-50 to-pink-50">
      ...
      {/* Header (emoji dan kategori sudah dihapus) */}
      ...
      {/* Services Grid — tombol diseragamkan */}
      {services.length > 0 && (
        <section className="py-20 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              ...
              {services.slice(0, 9).map((service) => (
                <Card 
                  key={service.id} 
                  className="group hover:shadow-2xl transition-all duration-300 overflow-hidden border-0 bg-white/80 backdrop-blur flex flex-col h-full"
                >
      ...
      <footer className="py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-r from-pink-600 to-purple-600 text-white text-center">
        ...
        <p className="text-sm opacity-80 mt-2">
          <Link href="/tenant/login" className="hover:underline font-medium text-pink-200">
