...
export function LocationMap({ 
  ...
  useEffect(() => {
    ...
    async function initializeMap() {
        ...
        if (typeof businessLocation === 'string') {
          ...
          try {
            ...
            const response = await new Promise<any>((resolve, reject) => {
              geocoder.geocode({ address: businessLocation }, (results: any, status: any) => {
      ...
      if (businessLocation && businessCoords && typeof businessCoords === 'object' && 'lat' in businessCoords && markersRef.current.length > 0) {
