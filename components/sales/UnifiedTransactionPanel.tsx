                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full bg-white border border-gray-200 flex items-center justify-center text-txt-secondary">
                                <i className='bx bx-hashtag text-lg'></i>
                            </div>
                            <div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="bg-white border border-gray-200 rounded-lg p-4">
                                    <div className="flex justify-between text-sm mb-2">
                                        <span className="text-txt-secondary">Paid: {formatCurrency(transaction.paidAmount)}</span>
                                        <span className="font-semibold text-txt-primary">Total: {formatCurrency(transaction.totalAmount)}</span>
                                    </div>
                                    <div className="w-full bg-gray-100 rounded-full h-2.5 overflow-hidden">
                                        <div
                                            className="bg-green-500 h-full transition-all duration-500"
                                            style={{ width: `${Math.min(100, ((transaction.paidAmount || 0) / transaction.totalAmount) * 100)}%` }}
                                        ></div>
                                    </div>
                                    {transaction.totalAmount > (transaction.paidAmount || 0) && (
                                        <p className="text-xs text-red-600 mt-2 text-right">
                                            Remaining: {formatCurrency(transaction.totalAmount - (transaction.paidAmount || 0))}
                                        </p>
                                    )}
                                </div>
                                        ...
                                        transaction.payments.map((payment, idx) => (
                                            <div key={idx} className="bg-gray-50 border border-gray-200 rounded-lg p-3 flex justify-between items-center">
                                                ...
                                                <div className="text-right">
                                                    <span className="font-bold text-green-600 text-sm">+ {formatCurrency(payment.paymentAmount)}</span>
