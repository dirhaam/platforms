-- Comprehensive RLS Policy Setup for BooQing Platform
-- This file creates secure, tenant-isolated row level security policies
...
-- Sessions table - needs special handling for cross-tenant access
CREATE POLICY "Sessions: Access allowed for valid session" ON sessions
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM tenants t
      WHERE t.id = tenant_id::text  -- Converting UUID to text for comparison
      AND (auth.jwt() ->> 'email' = t.email OR auth.uid()::text = user_id)
    )
    OR EXISTS (
      SELECT 1 FROM super_admins sa
      WHERE sa.email = auth.jwt() ->> 'email'
...
-- Step 3: Create policies for public access where needed (for website, etc.)
-- These might be needed for public-facing features:
CREATE POLICY "Public: Read-only access to public tenant info" ON tenants
  FOR SELECT USING (
    true  -- Public access to basic tenant information
  );
...
SELECT 'RLS policies have been configured successfully!' as status;