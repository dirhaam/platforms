...
async function fetchLegacyConversations(): Promise<LegacyConversationRow[]> {
  const { data, error } = await supabase
    .from<LegacyConversationRow>('whatsapp_conversations')
    .select('*')
...
async function fetchLegacyMessages(conversationId: string): Promise<LegacyMessageRow[]> {
  const { data, error } = await supabase
    .from<LegacyMessageRow>('whatsapp_messages')
    .select('*')
...
async function migrateConversation(conversationRow: LegacyConversationRow): Promise<{ migrated: number; conversationId: string }> {
  ...
  const messageRows = await fetchLegacyMessages(conversationRow.id);
...
async function main() {
  console.log('Starting WhatsApp history migration...');

  try {
    const conversations = await fetchLegacyConversations();
    if (conversations.length === 0) {
      console.log('No legacy conversations found. Nothing to migrate.');
      return;
    }
    ...
    for (const conversationRow of conversations) {
      try {
        const result = await migrateConversation(conversationRow);
        processedConversations += 1;
        totalMessages += result.migrated;

        console.log(
          `Conversation ${result.conversationId} processed (${result.migrated} messages migrated)`
        );
      } catch (conversationError) {
        console.error('Failed to migrate conversation:', {
